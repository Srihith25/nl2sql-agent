...
`Dockerfile` builds a single API-only image (Python 3.11, `pip install -e ".[anthropic]"`), seeds TPC-H and builds the vector index at build time, and runs `uvicorn app.api:api` on port 8000 — no Streamlit UI in the container. `ENV PYTHONPATH=/app` is required: `pip install -e .` runs before `COPY . .` (for layer caching), so the editable install doesn't reliably put `/app` on `sys.path` for a plain `python scripts/...` invocation or for the final `uvicorn app.api:api` CMD — without it both the build's index step and the running server fail with `ModuleNotFoundError: No module named 'app'`.

`render.yaml` deploys this image to Render's free tier with `LLM_PROVIDER=anthropic` and **`EMBEDDER=hash`** — deliberately not `[embed]`/sentence-transformers in production: `torch` + `sentence-transformers` (which pulls in a full unused CUDA toolkit) OOM-crashes Render's free 512MB instance the first time a session loads an embedder. `get_embedder()`'s `"auto"` mode would also silently fall back to the hash embedder if the package were ever missing, but don't rely on that in production — set `EMBEDDER=hash` explicitly. This does mean production schema retrieval is non-semantic (hash-based); if retrieval quality matters more than free-tier RAM, install `[embed]` and upgrade the Render plan instead.

The frontend is deployed separately (e.g. Vercel) pointing `NEXT_PUBLIC_API_URL` at the Render service — set the **Root Directory to `frontend`** in the Vercel project (ideally at import time, before the first deploy) or Vercel's zero-config detection will find the root `pyproject.toml` and try to treat the whole repo as a Python project.
